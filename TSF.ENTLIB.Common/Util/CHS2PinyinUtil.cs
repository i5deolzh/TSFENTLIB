...
namespace TSF.ENTLIB.Common.Util
{
    ...
    /// 汉字转拼音工具
    /// </summary>
    public sealed class CHS2PinyinUtil
    {
        /// <summary>
        /// 包含字符 ASC 码的整形数组
        /// </summary>
        ...
        public static string Convert(string chs, string separator, bool initialCap)
        {
            ...
            int i2 = 0;
            bool b = false;
            char[] chr = chs.ToCharArray();
            CultureInfo ci = System.Threading.Thread.CurrentThread.CurrentCulture;
            TextInfo ti = ci.TextInfo;

            for (int j = 0; j < chr.Length; j++)
            {
                arr = Encoding.Default.GetBytes(chr[j].ToString());
                string s = chr[j].ToString();
